...
@app.cell
def _(columns, pl):
    train = pl.read_csv("./assets/KaggleDataset/CMaps/train_FD004.txt", separator=" ", has_header=False, truncate_ragged_lines=True).drop(["column_27", "column_28"])
...
@app.cell
def _(mo, train_rul):
    train_rul.write_csv("./assets/KaggleDataset/train_rul_FD004_clean.csv")
    # test.write_csv("test_FD001_clean.csv")
    # rul_true.write_csv("rul_FD001_true.csv")

    mo.md("Fichiers exportés : train_rul_FD004_clean.csv")
    return
...
def _(sv, train_rul):
    ...
    reportCsvTrainNasa.show_html(filepath="./reports/reportCsvTrainNasa04.html", open_browser=False)
...
@app.cell
def _(columns, pl):
    test = pl.read_csv("./assets/KaggleDataset/CMaps/test_FD004.txt", separator=" ", has_header=False, truncate_ragged_lines=True, ).drop(["column_27", "column_28"])

    test.columns = columns

    rul_true = pl.read_csv("./assets/KaggleDataset/CMaps/RUL_FD004.txt", separator=" ", has_header=False, truncate_ragged_lines=True)
...
@app.cell
def _(mo, test):
    test.write_csv("./assets/KaggleDataset/test_FD004_clean.csv")
    # test.write_csv("test_FD001_clean.csv")
    # rul_true.write_csv("rul_FD001_true.csv")

    mo.md("Fichiers exportés : test_FD004_clean.csv")
    return


@app.cell
def _(mo, rul_true):
    rul_true.write_csv("./assets/KaggleDataset/rul_true_FD004_clean.csv")
    mo.md("Fichiers exportés : rul_true_FD004_clean.csv")
    return
...
def _(mo, train_clf):
    train_clf
    train_clf.write_csv("./assets/KaggleDataset/train_clf_FD004_clean.csv")
...
def _(mo, rf_clf, rf_reg):
    ...
    joblib.dump(rf_clf, "rf_classifier_FD004.joblib")
    joblib.dump(rf_reg, "rf_regressor_FD004.joblib")

    mo.md("**Modèles exportés :** `rf_classifier_FD004.joblib`, `rf_regressor_FD004.joblib`")
    return
