...
def _(columns, pl):
    train = pl.read_csv(
        "./assets/KaggleDataset/CMaps/train_FD004.txt",
        separator=" ",
        has_header=False,
...
def _(sv, train_rul):
    ...
    reportCsvTrainNasa.show_html(
        filepath="./reports/reportCsvTrainNasa04.html", open_browser=False
    )
...
def _(columns, pl):
    test = pl.read_csv(
        "./assets/KaggleDataset/CMaps/test_FD004.txt",
        separator=" ",
        has_header=False,
        truncate_ragged_lines=True,
    ).drop(["column_27", "column_28"])
    ...
    rul_true = pl.read_csv(
        "./assets/KaggleDataset/CMaps/RUL_FD004.txt",
        separator=" ",
        has_header=False,
        truncate_ragged_lines=True,
    )
...
def _(mo, rf_clf, rf_reg):
    ...
    mo.md(
        "**Modèles exportés :** `rf_classifier_FD004.joblib`, `rf_regressor_FD004.joblib`"
    )
    return
